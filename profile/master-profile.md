# Master Profile — Joshua Rotenberg

> Canonical source of truth for the `/tailor-resume` skill. Everything the tool
> uses to assemble a tailored resume must come from this file. **Do not invent
> experience, employers, dates, metrics, or skills that are not below.** Tailoring
> = selecting, reordering, and rephrasing TRUE content to match a job — never
> fabricating it.
...
| Field | Value |
| :--- | :--- |
| Name | Joshua Aaron Rotenberg |
| Location | Arvada, Colorado |
| Email | jrotnbrg@gmail.com |
| Phone | 303-726-5037 |
...
> sorted by end date — **a currently-held role always goes on top** (Solenzo above Level
> Agency, confirmed by Josh, settled). Roles may be *omitted* when they are
...
- Eliminated spam and junk leads on a Google Ads client by cleaning up the client's website (where the leads were coming in), then diagnosed on-site CRO as the remaining conversion bottleneck, demonstrating full-funnel troubleshooting.
  > **Correction (confirmed by Josh, Sep 2026):** the junk-lead fix was a website cleanup, not changes inside the Google Ads account. Josh's words: "all I did was clean the website up and got rid of them." Do not describe this as ad-account optimization, negative keywords, match-type or targeting changes. Earlier resumes (e.g. Craft & Communicate) used the old "optimized a client's Google Ads account" phrasing; use this version going forward.
...
| Role Type | Summary |
| :--- | :--- |
| GHL / HubSpot / Marketing Ops | Marketing operations leader and CRM architect with 15+ years in marketing and revenue operations, including 5+ years building CRM and marketing-automation systems — primarily in GoHighLevel, with hands-on HubSpot client builds. That builds on a decade in client-facing account roles where relationships and retention drove revenue. It's the same client-first instinct I bring to marketing operations for agencies, service-based, and B2B organizations. |
...
## Verified Quantified Metrics (the only hard numbers — use only these)
